...
| :--- | :--- | :--- |
| **1.9.0** | 2026-09-16 | **Vista de Detalle Total desde Stock ERP**: Se habilitó la interacción al hacer clic en el badge de cantidades totales (**TOTAL**) de cada producto en la tabla principal de inventario. Al hacer clic, consulta todos los registros de movimientos en `Stock_ERP` sin filtro de tipo de almacenaje, consolidando las cantidades por serie, depósito y tipo de almacenaje, con soporte para alternar entre el resumen agrupado y el **Kardex original completo**, visualización de registros en cero y filtros en memoria (incluyendo filtro exclusivo por tipo de almacenaje, serie y depósito). |
| **1.8.0** | 2026-08-24 | **Detalle Interactivo para Venta Sujeta a Confirmación (VTA)**: Se transformó el badge del campo `VTA.` en un botón interactivo cuando `venta_sujeta > 0`. Al hacer clic, consulta la tabla `Stock_ERP` filtrando por `tipo_almacenaje='VTA. SUJET. A CONF(MER)/BIENES DE USO'`, mostrando la tabla detalle con el título personalizado *"DETALLE DE VENTA SUJETA A CONFIRMACION"*, badge estilizado en paleta Teal (verde azulado) y recálculo de sumas en la cabecera. |
| **1.7.3** | 2026-08-14 | **Paginación Dinámica Ultra Rápida en Tránsito**: Se eliminó `top=1000` al ordenar por `TRANS.`, utilizando la paginación estándar de 60 en 60 registros devuelta directamente por `SI_Transito`. El cruce con `StockAprobado` se ejecuta en simultáneo por lotes de 10 productos para la página activa, manteniendo la respuesta veloz e interactiva para el usuario. |
| **1.7.2** | 2026-08-14 | **Consolidación de Tránsito y Consultas por Lotes (10 en 10)**: Se implementó la consolidación previa de `SI_Transito` por producto sumando `cant_pend`, seguida de consultas simultáneas a `StockAprobado` en lotes de máximo 10 productos (`&prod_id__codigo__in=...`). La vista muestra exclusivamente la tabla resumen de productos en tránsito ordenada por su cantidad pendiente acumulada. |
| **1.7.1** | 2026-08-14 | **Optimización de Indicador de Carga y Fallback en Tránsito**: Se añadió un indicador de carga animado en el cuerpo de la tabla (`<tbody>`) mientras se consulta `SI_Transito` y `StockAprobado`, evitando el mensaje falso de *"no se encontraron productos"*. Además, al ordenar por `TRANS.`, si los filtros activos no devuelven registros, se realiza la consulta completa a `StockAprobado` combinando y mostrando los 421 productos de tránsito de forma continua. |
| **1.7.0** | 2026-08-14 | **Ordenamiento Generalizado en Todas las Columnas Numéricas**: Se extendió la funcionalidad de ordenamiento por parámetro `ordering` a las 11 columnas numéricas de la tabla principal (`disponible`, `reservado`, `importacion`, `inkjet`, `acondicionado`, `reesterilizado`, `observados`, `consignacion`, `venta_sujeta`, `trans` y `stock`), manteniendo los tooltips y nombres intactos. Para la columna `TRANS.`, se consulta `SI_Transito` ordenado por `cant_pend` recuperando los productos coincidentes en `StockAprobado`. |
| **1.6.2** | 2026-08-14 | **Columna `RESERV.` (Mercadería Reservada)**: Se agregó la columna `RESERV.` entre `STOCK DISP.` y `PRE STOCK` en la tabla principal de Stock Aprobados con el tooltip *"Mercadería Reservada para Órdenes de Compra"*. Mapea y suma el campo `reservado` de la API de `StockAprobado` e incluye el campo en los reportes exportados a Excel. |
