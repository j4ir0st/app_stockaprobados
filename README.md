...
| :--- | :--- | :--- |
| **1.6.0** | 2026-08-10 | **Columna `TRANS.` (Mercadería en Tránsito)**: Se incorporó la columna `TRANS.` entre `VTA.` y `TOTAL` en la tabla principal de Stock Aprobados. La información se recupera asíncronamente desde `/SI_Transito/` con `top=1000`, mostrando un spinner animado (`fa-spinner fa-spin`) mientras se completa la consulta. Mapea y suma el campo `cantidad` por código coincidente o muestra `0` si no existen registros en tránsito. Incluye soporte para exportación a Excel. |
